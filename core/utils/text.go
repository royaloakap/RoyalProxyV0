...
func AskBool(question string, defaultVal bool) bool {
	...
	input := ReadTerminal()
	if input == "" {
		fmt.Println("[" + PrimaryColor("-") + "] [ " + PrimaryColor("Utilisation de la valeur par défaut "+fmt.Sprint(defaultVal)) + " ]")
		return defaultVal
	}
...
func AskInt(question string, defaultVal int) int {
	...
	if err != nil {
		fmt.Println("[" + PrimaryColor("!") + "] [ " + PrimaryColor("La réponse fournie n'est pas un nombre!") + " ]")
...
func AskString(question string, defaultVal string) string {
	...
	input := ReadTerminal()
	if input == "" {
		fmt.Println("[" + PrimaryColor("-") + "] [ " + PrimaryColor("Utilisation de la valeur par défaut "+defaultVal) + " ]")
		return defaultVal
	}
	return input
}
