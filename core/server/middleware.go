...
func Middleware(c *fiber.Ctx) {
	...
	log.Println("Requested domain:", domainName)
	firewall.Mutex.Lock()
	domainData := domains.DomainsData[domainName]
	firewall.Mutex.Unlock()

	if domainData.Stage == 0 {
		c.SendString("RoyalProxy: " + domainName + " n'existe pas. Si vous etes le proprietaire, veuillez verifier votre config.json si vous pensez qu il s'agit d une erreur contacter @Royaloakap")
		return
	}
	...
	if ipCountCookie > proxy.FailChallengeRatelimit {
		c.SendString("Bloque par RoyalProxy.\nVotre taux est limite. (R1)")
		return
	}

	//Ratelimit spamming Ips (feel free to play around with the threshhold)
	if ipCount > proxy.IPRatelimit {
		c.SendString("Bloqué par RoyalProxy.\nVotre taux est limite. (R2)")
		return
	}
	...
	if forbiddenFp != "" {
		c.SendString("Bloqué par RoyalProxy.\nTon navigateur " + forbiddenFp + " n'est pas autorisé.")
		return
	}
	...
	if !encryptedExists {
		...
		default:
			c.SendString("Bloqué par RoyalProxy.\nDemande de niveau suspecte " + susLvStr)
			return
		}
	...
	if !strings.Contains(cookieString, "__RoyalProxy_v="+encryptedIP) {
		...
		case 1:
			c.Append("Set-Cookie", "_1__RoyalProxy_v="+encryptedIP+"; SameSite=Lax; path=/; Secure")
		...
		case 3:
			...
			if !captchaExists {
				...
				if err := png.Encode(&buf, captchaImg); err != nil {
					c.SendString("Erreur RoyalProxy : échec de l'encodage du captcha: " + err.Error())
					return
				}
				data := buf.Bytes()
...
/* Add styles for the collapsible help text */ 
			...
			<body>
				<div class="center" id="center">
					<div class="box" id="box">
						<h1>Saisissez le <b>green</b> texte que vous voyez sur l'image</h1>  <canvas id="image" width="100" height="37"></canvas>
						<form onsubmit="return checkAnswer(event)">
							<input id="text" type="text" maxlength="6" placeholder="Solution" required>
							<button type="submit">Submit</button>
						</form>
						<div class="success" id="successMessage" style="display: none;">Success! Redirection ...</div>
						<div class="failure" id="failMessage" style="display: none;">Échoué! Veuillez réessayer.</div>
						<button class="collapsible">Pourquoi je vois cette page?</button>
			...
			function checkAnswer(event) {
				...
				document.cookie = '` + ip + `_3__RoyalProxy_v='+input+'` + publicPart + `; SameSite=Lax; path=/; Secure';
	...
	case "/_RoyalProxy/fingerprint":
		...
		return
	case "/_RoyalProxy/verified":
		c.SendString("verified")
		return
	case "/_RoyalProxy/" + proxy.AdminSecret + "/api/v1":
	...
	case "/_RoyalProxy/credits":
		c.SendString("RoyalProxy Lite; Lightweight http reverse-proxy https://t.me/Royaloakap. Protected by Roy GENERAL PUBLIC LICENSE Version 4, June 2024")
		return
	}
