...
func checkAttack(domainName string, domainData domains.DomainData) {
	...
	if !domainData.StageManuallySet || (domainData.BufferCooldown > 0) {
		...
		if !domainData.BypassAttack && !domainData.RawAttack && (domainData.BufferCooldown > 0) {
			...
			if domainData.BufferCooldown == 0 {
				utils.AddLogs("Webhook de fin d'attaque envoyé", "debug")
		...
		case 1:
			...
			if domainData.RequestsBypassedPerSecond > domainSettings.BypassStage1 && !domainData.BypassAttack {
				utils.AddLogs("Attaque de contournement lancée", "debug")
		...
		case 2:
			...
			if domainData.RequestsBypassedPerSecond > domainSettings.BypassStage2 {
				...
				utils.AddLogs("Attaque de contournement terminée", "debug")
		...
		if domainData.RequestsPerSecond > domainSettings.DisableRawStage2 && !domainData.RawAttack && !domainData.BypassAttack {
			utils.AddLogs("Attaque brute lancée", "debug")
			...
			utils.AddLogs("Attaque brute terminée", "debug")
			domainData.RawAttack = false
		}
...
func printStats() {
	...
	if domainData.Stage == 0 && proxy.WatchedDomain != "debug" {
		if proxy.WatchedDomain != "" {
			fmt.Println("[" + utils.PrimaryColor("!") + "] [ " + utils.PrimaryColor("Domain \""+proxy.WatchedDomain+"\" Pas trouvé") + " ]")
			fmt.Println("")
		}
		fmt.Println("[" + utils.PrimaryColor("Domaines disponibles") + "]")
		counter := 0
		...
		fmt.Println("[" + utils.PrimaryColor("Commandes disponibles") + "]")
		fmt.Println("")
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("help") + " ]: " + utils.PrimaryColor("Affiche toutes les commandes disponibles. Des informations plus détaillées peuvent être trouvées sur ") + "https://github.com/Royaloakap/RoyalProxy#commands")
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("stage") + " ]: " + utils.PrimaryColor("Usage: ") + "stage [number] " + utils.PrimaryColor("Verrouille la scène sur le numéro spécifié. Utiliser ") + "stage 0 " + utils.PrimaryColor("pour débloquer la scène"))
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("domain") + " ]: " + utils.PrimaryColor("Usage: ") + "domain [name] " + utils.PrimaryColor("Basculez entre vos domaines. Tapez uniquement ") + "domain " + utils.PrimaryColor("pour lister tous les domaines disponibles"))
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("add") + " ]: " + utils.PrimaryColor("Usage: ") + "add " + utils.PrimaryColor("Démarre un dialogue pour ajouter un autre domaine au proxy"))
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("rtlogs") + " ]: " + utils.PrimaryColor("Usage: ") + "rtlogs " + utils.PrimaryColor("Active et désactive les « Journaux en temps réel ». Il est suggéré d'éviter"))
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("clrlogs") + " ]: " + utils.PrimaryColor("Usage: ") + "clrlogs " + utils.PrimaryColor("Efface tous les journaux du domaine actuel"))
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("cachemode") + " ]: " + utils.PrimaryColor("Usage: ") + "cachemode " + utils.PrimaryColor("Active ou désactive la mise en cache du proxy"))
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("delcache") + " ]: " + utils.PrimaryColor("Usage: ") + "delcache " + utils.PrimaryColor("Efface le cache du domaine actuel"))
		fmt.Println("[" + utils.PrimaryColor("+") + "] [ " + utils.PrimaryColor("reload") + " ]: " + utils.PrimaryColor("Usage: ") + "reload " + utils.PrimaryColor("Rechargez votre proxy pour que les changements dans votre ") + "config.json " + utils.PrimaryColor("prendre effet "))
	} else {
		...
		fmt.Println("")
		fmt.Println("[ " + utils.PrimaryColor("Creer Par Royaloakap") + " ]")
		fmt.Println("[ " + utils.PrimaryColor("Dernière logs") + " ]")
		...
		if scanner.Scan() {
			...
			case "rtlogs":
				...
				if proxy.RealTimeLogs {
					...
					fmt.Println("[ " + utils.PrimaryColor("Désactiver les journaux en temps réel") + " ] ...")
				} else {
					proxy.RealTimeLogs = true
					fmt.Println("[ " + utils.PrimaryColor("Activation des journaux en temps réel") + " ] ...")
				}
				fmt.Println("\033[" + fmt.Sprint(12+proxy.MaxLogLength) + ";1H")
				fmt.Print("[ " + utils.PrimaryColor("Command") + " ]: \033[s")
			case "clrlogs":
				...
				if proxy.WatchedDomain == "" {
					...
					fmt.Println("[ " + utils.PrimaryColor("Effacement des journaux de tous les domaines ") + " ] ...")
				} else {
					firewall.Mutex.Lock()
					utils.ClearLogs(proxy.WatchedDomain)
					firewall.Mutex.Unlock()
					fmt.Println("[ " + utils.PrimaryColor("Effacement des journaux pour "+proxy.WatchedDomain) + " ] ...")
				}
				fmt.Println("\033[" + fmt.Sprint(12+proxy.MaxLogLength) + ";1H")
				fmt.Print("[ " + utils.PrimaryColor("Command") + " ]: \033[s")
			case "reload":
				screen.Clear()
				screen.MoveTopLeft()
				fmt.Println("[ " + utils.PrimaryColor("Rechargement de RoyalProxy") + " ] ...")
				reloadConfig()
				fmt.Println("\033[" + fmt.Sprint(12+proxy.MaxLogLength) + ";1H")
				fmt.Print("[ " + utils.PrimaryColor("Command") + " ]: \033[s")
			case "help":
...
func clearProxyCache() {
	...
	for {
		...
		if (proxyCpuUsage < 15 && proxyMemUsage > 25) || proxyMemUsage > 95 {
			...
			utils.AddLogs("Adresses IP mises en cache effacées", "debug")
		} else {
			utils.AddLogs("N'a pas effacé les adresses IP mises en cache", "debug")
		}
		// Same for here
		imgCachelen := 0
		...
		if (proxyCpuUsage < 15 && proxyMemUsage > 25) || proxyMemUsage > 95 {
			...
			utils.AddLogs("Captchas mis en cache effacés", "debug")
		} else {
			utils.AddLogs("N'a pas effacé les captchas mis en cache", "debug")
		}
		firewall.Mutex.Unlock()
